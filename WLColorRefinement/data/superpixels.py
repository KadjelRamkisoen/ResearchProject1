...
class SuperPixDGL(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if dataset == 'MNIST':
            self.img_size = 28
            with open(os.path.join('C:\\Users\\User1\\Documents\\GitHub\\ResearchProject1\\WLColorRefinement\\data\\superpixels', 'mnist_75sp_%s.pkl' % split), 'rb') as f:
                self.labels, self.sp_data = pickle.load(f)
                self.graph_labels = torch.LongTensor(self.labels)
        elif dataset == 'CIFAR10':
            self.img_size = 32
            with open(os.path.join('C:\\Users\\User1\\Documents\\GitHub\\ResearchProject1\\WLColorRefinement\\data\\superpixels', 'cifar10_150sp_%s.pkl' % split), 'rb') as f:
                self.labels, self.sp_data = pickle.load(f)
                self.graph_labels = torch.LongTensor(self.labels)
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        self.name = name
        data_dir = 'C:\\Users\\User1\\Documents\\GitHub\\ResearchProject1\\WLColorRefinement\\data\\superpixels'
